...
export class TournamentService {
  ...
  async createTournament(data: TournamentCreateRequest): Promise<{ id: string; name: string; maxPlayers: number }> {
  ...
  async joinTournament(tournamentId: string, alias: string): Promise<{ success: boolean; message?: string }> {
    const tournament = await this.db.get(`
  ...
  async startTournament(tournamentId: string): Promise<{ success: boolean; bracket?: any; message?: string }> {
    const tournament = await this.db.get(`
      SELECT id, max_players, current_players, status 
      FROM tournaments WHERE id = ?
    `, [tournamentId]);
