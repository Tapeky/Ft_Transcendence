...
async function migrateGitHub() {
  ...
  try {
    ...
    const finalTableInfo = await db.all("PRAGMA table_info(users)");
    const finalIndexes = await db.all("PRAGMA index_list(users)");
    
    console.log('📋 Colonnes de la table users:');
    finalTableInfo.forEach((col: any) => {
      if (col.name.includes('github') || col.name === 'id' || col.name === 'email') {
        console.log(`   - ${col.name}: ${col.type} ${col.notnull ? 'NOT NULL' : ''} ${col.pk ? 'PRIMARY KEY' : ''}`);
      }
    });
    
    console.log('📋 Index sur la table users:');
    finalIndexes.forEach((idx: any) => {
      if (idx.name.includes('github')) {
