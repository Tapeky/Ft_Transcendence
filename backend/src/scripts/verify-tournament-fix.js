...
async function runTests() {
  try {
    ...
    // Check compatibility
    const isCompatible = (tournamentsIdCol.type.includes('INTEGER') && 
                         playersIdCol.type.includes('VARCHAR')) ||
                        (tournamentsIdCol.type.includes('INTEGER') && 
                         matchesIdCol.type.includes('VARCHAR'));
    ...
    if (isCompatible) {
      ...
      console.log('   Either change tournament_players.tournament_id and tournament_matches.tournament_id to INTEGER,');
      console.log('   or change tournaments.id to VARCHAR(36) and update the backend code accordingly.');
