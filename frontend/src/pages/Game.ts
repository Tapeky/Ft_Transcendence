...
export class GamePage {
  ...
  private setupLocalGame(): void {
    this.gameState = {
      leftPaddle: {
        pos: { x: 20, y: this.ARENA_HEIGHT / 2 - this.PADDLE_HEIGHT / 2 },
        hitCount: 0,
      },
      rightPaddle: {
        pos: { x: this.ARENA_WIDTH - 28, y: this.ARENA_HEIGHT / 2 - this.PADDLE_HEIGHT / 2 },
        hitCount: 0,
      },
      ball: {
        pos: { x: this.ARENA_WIDTH / 2, y: this.ARENA_HEIGHT / 2 },
        direction: { x: this.INITIAL_BALL_SPEED, y: 3 },
      },
      state: 1,
      leftScore: 0,
      rightScore: 0,
  ...
  private updateLocalPaddles(): void {
    ...
    if (
      this.localInputLeft.down &&
      this.gameState.leftPaddle.pos.y < this.ARENA_HEIGHT - this.PADDLE_HEIGHT
    ) {
      this.gameState.leftPaddle.pos.y += speed;
    }
    ...
    if (
      this.localInputRight.down &&
      this.gameState.rightPaddle.pos.y < this.ARENA_HEIGHT - this.PADDLE_HEIGHT
    ) {
      this.gameState.rightPaddle.pos.y += speed;
    }
  }
  ...
  private render(): void {
    ...
    this.ctx.fillText(this.gameState.leftScore.toString(), this.ARENA_WIDTH / 4, 80);
    this.ctx.fillText(this.gameState.rightScore.toString(), (3 * this.ARENA_WIDTH) / 4, 80);
    if (this.tournamentContext) {
      this.ctx.font = '24px Iceland, monospace';
      this.ctx.fillStyle = '#ADD8E6';
      this.ctx.fillText(this.tournamentContext.player1Alias, this.ARENA_WIDTH / 4, 120);
      this.ctx.fillText(this.tournamentContext.player2Alias, (3 * this.ARENA_WIDTH) / 4, 120);
    }
    if (this.isCountingDown) {
      ...
      this.ctx.textAlign = 'center';
      this.ctx.fillText('GET READY!', this.ARENA_WIDTH / 2, this.ARENA_HEIGHT / 2 - 80);
      if (this.tournamentContext) {
        this.ctx.fillStyle = '#ADD8E6';
