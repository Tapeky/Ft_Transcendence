...
class ApiService {
  ...
  async sendFriendRequest(
    friendId: number
  ): Promise<{ id: number; friend_id: number; status: string }> {
    const response = await this.request<{ id: number; friend_id: number; status: string }>(
      '/api/friends/request',
      {
        method: 'POST',
  ...
  async setAvatar(avatarId: string): Promise<{ avatar_id: string; avatar_url: string }> {
    const response = await this.request<{ avatar_id: string; avatar_url: string }>(
      '/api/avatars/set',
      {
        method: 'PUT',
  ...
  async uploadAvatar(file: File): Promise<{ avatar_url: string; filename: string }> {
    ...
    const response = await this.request<{ avatar_url: string; filename: string }>(
      '/api/avatars/upload',
      {
        method: 'POST',
        body: formData,
  ...
  async getMatches(params?: {
    ...
    if (params?.include_guests)
      searchParams.append('include_guests', params.include_guests.toString());
    if (params?.include_stats)
      searchParams.append('include_stats', params.include_stats.toString());
    const response = await this.request<{
      data: Match[];
      pagination: { limit: number; offset: number; total: number };
    }>(`/api/matches${searchParams.toString() ? '?' + searchParams.toString() : ''}`);
    return response.data!;
  }
