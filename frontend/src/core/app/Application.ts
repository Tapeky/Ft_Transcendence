...
export class Application {
  ...
  private initializeGameInviteSystem(): void {
    ...
    this.authManager.onAuthStateChange((user) => {
      if (user) {
        // User is authenticated - KISS system handles invitations
        console.log('📋 User authenticated - KISS invitation system active');
      } else {
        // User logged out - KISS system handles cleanup
        console.log('📋 User logged out - KISS invitation system inactive');
      }
    });
  }
